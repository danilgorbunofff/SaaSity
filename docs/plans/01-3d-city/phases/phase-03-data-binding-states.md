...
1. **Fetch & hydrate**
   - Load plots via the shared types from 0.3; client store (Zustand) holds `plots by id` + loading/error states — the per-plot shape now includes the richer `LIVE` fields (`currentPriceCents`, `endAt`, plus `currentLeaderPreBidId` for ownership check) alongside the always-present tier/origin/span/status
   - > **Correction (Part 1 lifecycle fix, M2):** the brand snapshot is NOT a `LIVE`-only field — it is `tenant`, gated on `tenantPreBidId` independent of `status`, so a plot's last paid winner keeps displaying through `IDLE`. See `docs/reviews/m0-m2-remediation/part-01-product-lifecycle.md`.
   ...
   - Derive `ownedLeading: boolean` per plot as `plot.status === LIVE && myPreBids.some(pb => pb.cycleId === plot.currentCycleId && pb.id === plot.currentLeaderPreBidId)` — never trust a client-side flag from the public plots API alone
...
2. **Status skins (base layer, everyone sees)**
   - `IDLE`: base look, soft inviting emissive edge — genuinely empty when there is no standing tenant, matches "cheap and available right now". > **Correction (Part 1 lifecycle fix, M2):** an `IDLE` plot is NOT always empty — a plot with a paid tenant keeps showing their billboard while `IDLE` (claimable again, but still leased) until a new winner is activated.
...
3. **Personal identity layer (only on plots you currently lead)**
   ...
   - **Persistent billboard badge:** high-contrast floating pill `★ YOUR HQ: {companyName} • {countdown}` hovering above roof at all zoom levels using `drei <Html transform>` / `<Billboard>`. Countdown is coarse here (mm:ss, updated on the shared 5s tick, not per-second) — the precise per-second countdown lives only in 1.4's card. Badge auto-hides behind occlusion is acceptable; the beacon + aura already guarantee discoverability when it is. > **Correction (Part 1 lifecycle fix, M2):** shipped copy is `★ LEADING • {countdown} left`, not `★ YOUR HQ` — this badge means "you are the current bidding leader," which is provisional and unpaid; it must read differently from paid-tenant display copy so bidders never mistake leading for winning
   - **Outbid transition (state flip):** when a plot was `ownedLeading` and ceases to be (rival's `Bid` moves `currentLeaderPreBidId` away, observed via 2.4 realtime or next fetch), the badge **flips within one tick** from cyan `★ LEADING` to **flashing amber** `⚠️ OUTBID: +$X to retain` (where `X` = tier increment: $0.50 / $1.00 / $5.00). Aura ring switches to amber pulse, beacon dims/off. No new API — derived purely from `ownedLeading` going false while the cycle is still `OPEN`
...
- Dev-only: force `ownedLeading` on 2 plots → beacon + aura + badge appear, then simulate outbid (flip `currentLeaderPreBidId`) → badge flips to amber `OUTBID` within one tick
