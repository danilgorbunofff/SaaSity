...
**Status:** ✅ Done (review pass complete: minimap outbid state, My Leases empty state, detail-card outbid variant, toast tier-increment, TopStrip per-tier counts + legend, tick scoping) · **Estimate:** ~2 days
...
3. **Detail card**
   ...
   - > **Correction (Part 1 lifecycle fix, M2):** the card now renders two independent sections instead of one status-gated block. `TenantMeta` (paid, standing tenant — company name/tagline/handle/`mrrText`/site link) shows whenever `plot.tenant` exists, `LIVE` or `IDLE` alike. `AuctionMeta` (current price, countdown, current bidding leader as a provisional/unpaid pointer, "Place a bid" CTA) shows only when `status === 'LIVE'`. A contested plot with a standing tenant shows **both** at once — the old design's premise that "`LIVE` ⇒ show the leader's brand" and "`IDLE` ⇒ always empty, claim CTA only" is what caused the core lifecycle bug (winner's brand flickered to whoever was merely leading, and vanished the moment the plot went back to `IDLE`). See `docs/reviews/m0-m2-remediation/part-01-product-lifecycle.md`.
4. **City HUD — activity + navigation (the identification system)**
   ...
   - > **Correction (Part 1 lifecycle fix, M2):** shipped `N` = count of plots where **`isTenant`** is true (a paid, activated lease), not `ownedLeading` (merely leading an in-progress auction). "My Leases" naming a bidding position you haven't paid for yet was the exact confusion this fix removes — `ownedLeading` still exists and still drives the in-scene beacon/badge/minimap star, it's just no longer what this pill counts.
   - **2D radar minimap (new, bottom-right HUD):** flat **10×10** grid overlay mapping **1:1** to the 3D world grid (cell `(x, y)` in minimap = `originX/Y` in world). Visual encoding:
     ...
     - **Flashing amber `⚠️`:** plots where you were just outbid (you have a PreBid on the cycle but `currentLeaderPreBidId !== yours` and cycle still `OPEN`)
...
- [x] Minimap renders 10×10 correctly, four symbol states accurate, click flies to building at close-up inspect distance
